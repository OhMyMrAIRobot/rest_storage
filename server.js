...
app.post('/files/:filename/copy', async (req, res) => {
    ...
    let destFilePath = path.join(FILE_STORAGE_DIR, req.body.destination);

    try {
        await fs.access(srcFilePath, fs.constants.F_OK);
        const dirExists = await fs.access(destFilePath).then(() => true).catch(() => false);
        if (!dirExists) {
            await fs.mkdir(destFilePath, { recursive: true });
        }

        destFilePath = path.join(destFilePath, req.params.filename);
